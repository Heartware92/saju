...
10) ★★★ 섹션 간 중복 절대 금지 — 직원 피드백 핵심 항목.
    [원칙]
    - 한 풀이 안의 12개 섹션은 서로 다른 시각·다른 일상 장면·다른 결론을 제시해야 한다.
    - 같은 명리 근거(용신·격국·신살·합충 등)는 여러 섹션에 등장할 수 있으나
      **같은 표현·같은 장면·같은 결론으로 반복하면 안 된다.**

    [섹션별 고유 ownership — 각 섹션은 아래 영역에 집중]
    - general    : 격국 + 신강신약이 만드는 인생 큰 그림 (직업·재물·관계는 언급만, 깊이 다루지 X)
    - daymaster  : 일주 60갑자 고유 기질 (다른 섹션에서 일주 기질을 다시 풀지 말 것)
    - element    : 오행 분포 + 결핍/과다 (다른 섹션은 오행 한 줄 언급만)
    - interaction: 합·충·형·파·해 관계 (다른 섹션에서 합충 반복 금지)
    - character  : 낯선 자리 vs 친한 자리 성향 (general 의 격국과 겹치지 않게)
    - career     : 직업·적성·조직 역할 (재물 얘기는 wealth 로 미루기)
    - wealth     : 돈 흐름·재물 함정 (직업 얘기는 career 로 미루기)
    - love       : 끌리는 상대·관계 패턴 (가족·친구는 relation 으로)
    - health     : 약한 장부·건강 습관 (성격으로 인한 스트레스는 character 로)
    - relation   : 인맥·부모/자녀/귀인 (배우자·연애는 love 로)
    - luck       : 대운 흐름 (현재 시점 조언만, 평생 성향은 general 로)
    - advice     : 용신 보강 실천 (다른 섹션의 결론을 단순 반복 X, 구체 행동만)

    [금지 패턴 예시]
    - 같은 인용 반복: career 에서 "용신이 화이므로 봄에 좋다" → wealth 에서 또 "용신 화가 봄에..."
    - 같은 장면 반복: love 에서 "회의실에서 발표하는 모습" → career 에서도 "회의실 발표"
    - 같은 결론 반복: 모든 섹션이 "지금이 적기다" 또는 "조심해야 한다"로 끝나면 안 됨
    - 섹션 ownership 침범: wealth 섹션에서 "직장에서 승진할 시기" 길게 다루기 (X — career 영역)

    [작성 시 자기 검열]
    - 새 섹션 시작 전, 이전 섹션에서 쓴 핵심 문장·장면·결론을 스스로 떠올리고
      다르게 풀어 쓸 것. 같은 데이터라도 그 섹션의 주제에 맞춰 새로운 일상 장면을 발굴해야 한다.
