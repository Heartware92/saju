...
- 기신 = 달빛에 눌려 다른 별이 안 보이는 상태.
- 천을귀인 = 흐린 밤에도 유독 밝게 빛나는 행운의 별.
...
[제목 작성 기술]
[general] 섹션 작성 전 은유 제목을 한 줄 씁니다.
형식: 「대비되는 두 이미지를 쉼표로 연결」
예: "서리 내린 새벽, 그 아래 피어나는 봄꽃" / "겨울 밤 은하수, 그 속에 숨은 여름 태양"
제목 은유를 본문 첫 문장·중간·마지막에 재등장시킵니다.
...
[advice] — 300~380자
작성 순서:
첫 줄: 은유 제목 (용신 오행의 방향과 개운 에너지를 자연 이미지로. 예: "북극성이 가리키는 동쪽, 봄 햇살이 기다리는 곳" / "서리를 녹이는 것은 불꽃이 아니라 봄비")
빈 줄
본문: 제목 은유로 시작해 용신(${yongSinElement})을 일상에서 보강하는 구체적 방법:
- 색 2개(의류·소품·인테리어에 자주 쓸 것)
- 방향 1개(이사·사무실 배치·좌석 선택 시)
- 유리한 시간대 1구간(중요한 일을 배치하면 좋은 하루 중 구간)
- 식재료 또는 음식 1~2개(오행 보강 기준)
이번 달 안에 당장 실천 가능한 구체 행동 3가지를 불릿(- )으로. 추상적 격언 금지, 오늘·이번 주·이번 달 수준의 행동 단위로. 마지막 문장에서 제목 은유 회수.
