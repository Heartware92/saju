 ...
 */
export const METAPHOR_TITLE_RULE = `[섹션 은유 부제목 — 절대 규칙]
모든 ▶ 섹션의 첫 줄은 반드시 은유적 부제목(7~20자)으로 시작합니다. 본문 내용은 부제목 다음 줄부터.

형식:
▶ 섹션 제목 (분량)
부제목 은유 (자연·우주·계절 이미지로 이 섹션 핵심을 한 줄 시각화)
본문 시작...

부제목 작성법:
- 대비되는 두 이미지를 쉼표로 연결: "서리 내린 새벽, 그 아래 피어나는 봄꽃"
- 또는 하나의 선명한 장면: "두 개의 별이 같은 궤도를 도는 밤"
- KB 별칭을 그대로 복사하지 말 것. 섹션 내용에 맞는 고유한 이미지를 만들 것.
- 같은 풀이 안에서 부제목끼리 이미지가 겹치면 안 됨 (다양한 자연 소재 활용: 바람·파도·별·달·안개·눈·비·뿌리·불꽃·호수·숲·이슬 등).
...
▶ 갈등·마찰 포인트
같은 하늘 아래 엇갈리는 두 줄기 바람
두 사람 사이에서…(본문 시작)

부제목이 없는 섹션이 하나라도 있으면 잘못된 풀이로 간주됩니다.`;
...
export const TODAY_SCORE_DOMAINS = ['overall', 'wealth', 'work', 'love', 'health'] as const;
export type TodayScoreDomain = typeof TODAY_SCORE_DOMAINS[number];
export const TODAY_SCORE_LABELS: Record<TodayScoreDomain, string> = {
  overall: '종합',
  wealth:  '재물',
  work:    '업무',
  love:    '관계',
...
2) 각 섹션 분량 지침 준수. 총 1200~1600자 (4개 층 모두 다루므로 분량 확장).
...
7) 출력은 [today_scores] 마커부터 시작. 마커 이전 텍스트 없어야 함.
8) 아래 마커를 정확히 사용. 마커는 줄 처음에 단독으로 위치.

${METAPHOR_KB}

[점수 출력 규칙 — 반드시 가장 먼저 출력]
출력의 첫 번째 줄은 반드시 아래 형식이어야 합니다:
[today_scores] 종합:XX 재물:XX 업무:XX 관계:XX 건강:XX
- 각 점수는 0~100 정수. 반드시 사주 원국 + 대운 + 세운 + 월운 + 일진을 근거로 산출.
- 점수 분포 규칙 (★중요):
  · 5개 점수의 표준편차가 10 이상이어야 합니다. 비슷한 점수 나열 금지.
  · 최고점과 최저점의 차이가 25 이상이어야 합니다.
  · 용신이 일진에서 힘을 받으면 종합 70+ / 기신이 강하면 종합 50 이하.
  · 합(合)이 있으면 해당 영역 +10~15 / 충(沖)이 있으면 해당 영역 -10~15.
  · 50점대에 모든 점수가 몰리는 것은 잘못된 평가입니다. 일진의 영향을 적극 반영하세요.
  · 종합 점수는 나머지 4개의 가중 평균이 아닌, 전반적 기운의 독립 평가입니다.
...
[today_energy] — 200~280자 (은유 제목 별도, 본문 분량만)
첫 줄: 은유 제목 (오늘 기운의 핵심 성질을 자연 이미지 대비로)
본문: 「대운(${daeWoonStr.split('(')[0]}) → 세운(${seWoon.gan}${seWoon.zhi}) → 월운(${_mGan}${_mZhi}) → 일진(${todayGz.gan}${todayGz.zhi})」 4개 층이 겹쳐 만드는 오늘의 기운을 2~3문장으로 단정적으로 짚는다. 그 다음 오늘 어떤 상황에서 유리·불리한지 각 1개 구체적으로. 오늘 하루 키워드 1개로 마무리.

[today_work] — 280~360자
첫 줄: 은유 제목 (일·집중의 결을 자연 이미지 대비로)
본문: 일진(${todayGz.gan}${todayGz.zhi}) + 월운(${_mGan}${_mZhi}) 조합이 오늘의 업무에 어떻게 작용하는지 2문장. 집중이 잘 되는 업무 유형과 막히는 유형을 구체적으로. 오늘 하면 좋은 행동 1개(회의·기획·제출·연락·협상·계약 중 + 구체적 이유). 피해야 할 행동 1개(구체 상황 + 이유). 오늘 가장 생산적인 시간대 1구간과 그 이유. 재물운 흐름을 1문장으로 언급(수입·지출·투자 중 오늘 유리한 방향).

[today_love] — 200~280자
첫 줄: 은유 제목 (관계 온도·흐름을 자연 이미지 대비로)
본문: 오늘 일진 십성(${todayGz.tenGodGan}) 기준으로 잘 통하는 관계 유형과 마찰이 생기기 쉬운 관계 유형 각 1개. 조심할 말투·상황 1가지(구체 예시 포함). 연인·가족·동료 중 오늘 특히 신경 써야 할 관계 1가지와 실천할 행동 1가지. 오늘 인연이 들어오기 쉬운 상황이나 장소 1가지.

[today_caution] — 180~240자
첫 줄: 은유 제목 (오늘의 함정·빈틈을 자연 이미지로)
본문: 오늘 합충(${interStr})에서 생기는 실수 유발 상황 1가지를 구체적 장면으로. 건강·컨디션 면에서 오늘 취약한 부위나 증상 1가지(오행 근거). 특히 조심해야 할 시간대·감정 상태·물리적 환경 중 1개 선택해 구체화. 이미 벌어졌을 때 대처 방법 1문장.

[today_lucky] — 200~280자
...
빈 줄
본문: 제목 은유로 시작해 일주 ${pillars.day.gan}${pillars.day.zhi}(${dayTraits?.hanja ?? ''})의 고유한 에너지를 "이 일주를 타고난 사람은 ~한 방식으로 세상을 경험한다"는 관점에서 깊이 서술.
- DB 키워드(${dayTraits?.keywords?.join(', ') ?? ''})를 개별 나열 X — 이야기 흐름에 녹여 쓸 것
- 이 일주가 강점을 발휘하는 전형적 장면 2개와 발목 잡는 패턴 2개를 각각 구체적 상황으로 묘사
- 특수신살(${dayTraits?.sinsal?.join(', ') || '없음'})의 실생활 의미 2~3문장
- 다른 일주와 무엇이 결정적으로 다른지 1문장
마지막 문장에서 제목 은유 회수해 일주의 핵심 압축.

[element] — 540~660자
작성 순서:
첫 줄: 은유 제목 (오행 과다·결핍을 빛·그림자·계절 이미지로 대비. 예: "한쪽에서만 쏟아지는 햇살, 반대편의 짙은 그늘")
빈 줄
본문: 제목 은유로 시작해 오행 분포(목${elementPercent.목}% 화${elementPercent.화}% 토${elementPercent.토}% 금${elementPercent.금}% 수${elementPercent.수}%)를 풀어 쓴다.
- 사계절·하늘 은유(목=봄 새벽, 화=정오, 토=환절기, 금=서리 새벽, 수=겨울 은하수) 활용
- 결핍 오행(${zeroElements.join('·') || '없음'})의 구체 생활 약점 3가지 + 각 보완 방법
- 과다 오행(${maxEl[0]} ${maxEl[1]}%) 편향성이 드러나는 장면 2개
...
빈 줄
본문: 제목 은유로 시작해 일상 행동 패턴 중심으로 서술. (격국·일주 자체 묘사는 1차에서 다뤘으니 여기선 사용 X)
- "낯선 환경에서 처음 보이는 모습" 2~3문장 (구체 상황: 회의·소개팅·면접 등)
- "친해진 뒤 드러나는 본모습" 2~3문장 (구체 상황: 가까운 사람과의 갈등·기쁜 순간)
- 강점 2가지 + 그림자 2가지 (모두 일상 장면으로)
...
빈 줄
본문: 제목 은유로 시작해 적합한 직군 5~6개를 구체적으로(예: "IT 개발 중에서도 백엔드·시스템 설계 분야") 제시.
- 각 직군마다 "왜 이 사주에 맞는가" 한 문장 근거
- 조직 내 역할(리더형·참모형·독립형 중 선택) + 커리어 성장 패턴 2~3문장
- 피해야 할 직군·환경 2개 + 이유
...
[wealth] — 600~720자
작성 순서:
첫 줄: 은유 제목 (재물이 모이는 방식과 새는 패턴을 빛·물·계절로 대비)
빈 줄
본문: 제목 은유로 시작해 재성(편재·정재) 강약 + 재고 포함 여부 근거로 돈 버는 스타일 단정.
- 월급형·사업형·투자형 중 어느 쪽이 유리한가 + 근거
- 돈이 새는 패턴 + 쌓이는 패턴 각 1~2문장
- 반복되는 금전 함정 2가지 (구체 상황) + 각 예방법
- 큰 돈이 들어오는 시기·환경 단서 1문장
마지막 문장에서 제목 은유 회수.
...
빈 줄
본문: 제목 은유로 시작해 관성(여성)·재성(남성) 강약 + 위치로 끌리는 상대 유형.
- 어떤 분위기·말투·에너지 수준의 사람에게 반응하는가 (구체적으로)
- 일지 12운성이 배우자궁에 미치는 영향 1~2문장
- 유리한 대운 구간 연애·결혼 기회 (시기 명시)
- 반복되는 갈등 패턴 1개 + 개선 포인트
- 결혼 후 관계 안정화 키워드 1~2문장
마지막 문장에서 제목 은유 회수.
...
빈 줄
본문: 제목 은유로 시작해 약한 오행·충 받은 오행 기준 취약 장부 명시(목=간담, 화=심장·소장, 토=비위·췌장, 금=폐·대장, 수=신장·방광).
- 취약 장부의 스트레스 상황 증상 2~3가지 구체 묘사
- 일상 챙겨야 할 습관 3가지 + 하지 말아야 할 것 1가지
- 이 사주 기질이 만드는 건강 리스크 1~2가지
- 환절기·계절별 주의점 1문장
마지막 문장에서 제목 은유 회수.
...
빈 줄
본문: 제목 은유로 시작해 비겁·식상·관성 배치로 본 인맥 형성 스타일.
- "처음 만난 자리 행동" 1~2문장 + "어떤 관계에서 오래 유지되는가" 1~2문장
- 부모 관계 (인성·관성 근거) 1~2문장
- 자녀 관계 (식상 근거) 1~2문장 (시간 미상이면 "시주 미상으로 자녀궁 제한적" 한 줄)
- 의지하면 좋은 사람 유형 1개 + 거리 두어야 하는 유형 1개를 십성 근거로
마지막 문장에서 제목 은유 회수.
...
이전 대운(${prevDaeWoonStr})이 어떤 시기였는지 + 그 시기 형성된 기반·미해결 과제 1~2가지. 첫 대운이라 이전 없으면 "대운 시작 전 청소년기는 사주 원국이 그대로 발현되던 잠재기".

[2단락 — 현재 대운 본론] 480~600자
제목 은유로 시작해 현재 대운(${currentDaeWoonStr}) 선언. 그 대운의 간지·오행·십성·12운성이 일·관계·재물 각각에 어떤 영향을 주는지 5~6문장. 어떤 조건에서 유리/불리한지. 향후 5년 세운(${recentSeWoon}) 5개 연도 각각 한 줄씩 "YYYY년 OO(간지·십성)은 ~한 흐름이 들어와 ~을 우선해야 한다" 형식 (5줄 모두 필수).

[3단락 — 미래 대운 예고] 320~400자
다음 대운(${nextDaeWoonStr}) 어떤 국면이 열리는지 3~4문장 + 그 대운에서 가장 중요한 준비 한 가지를 지금부터 무엇으로 시작해야 하는지. 차차기 대운(${nextNextDaeWoonStr})까지 데이터 있으면 "그 다음 대운에선 ~" 한 줄로 예고. 데이터 끝이면 "그 너머는 본 사주 데이터 범위 밖" 명시.
...
빈 줄
(본문 350~450자: 제목 은유로 시작해 용신 ${yongSinElement} 보강의 의미와 일상 적용을 깊이 서술. 다른 섹션 결론 단순 반복 X, 처방 행동의 명리 근거에 집중.
...
★ 궁합 점수 산출 기준 (종합 점수를 산출할 때 이 프레임을 참고):
- 90~100: 일지합 + 용신 상호충족 + 오행 상보 + 충·형 없음
- 75~89: 일간 상생 + 지지합 1개 이상 + 약한 충돌 있으나 보완 가능
- 60~74: 특별한 합 없으나 충돌도 약함 + 부분적 오행 보완
- 45~59: 일지충 또는 용신·기신 충돌 + 오행 편향 겹침
- 30~44: 다중 충·형 + 기신 직접 충돌
- 0~29: 전면적 상극 + 합 부재 + 기신 쌍방 충돌
※ 항상 긍정적 가능성을 함께 제시. 낮은 점수도 "불가능"이 아닌 "다른 방식으로 풀어야 할 관계"로 서술.
...
- 각 ▶ 섹션의 첫 줄은 반드시 은유적 부제목(7~20자)으로 시작. 본문은 그 다음 줄부터.
- 출력은 첫 줄에 관계를 상징하는 은유 제목(7~14자)으로 시작. 대괄호·섹션 태그·식별자는 절대 출력하지 말 것. 총 분량: 2,400~3,200자.
...
▶ 핵심 요약 (200~280자)
두 일간 오행 관계(${elRel})를 근거로 이 커플의 에너지 구조를 한마디로 선언하세요. 일지 음양합(${eumYangHap})과 배우자성 오행 대응(${mySpouseCheck})으로 인연의 강도를 구체적으로 제시. 이 두 사람의 관계를 자연 현상이나 사계절 장면으로 비유하되, "왜 이 비유인지" 명리적 근거를 1문장 덧붙이세요. 마지막에 이 관계의 핵심 키워드 3개를 짧게 나열.
...
- 수치·판정 변경 금지. 흐린 표현 2회 이하.
- 각 ▶ 섹션의 첫 줄은 반드시 은유적 부제목(7~20자)으로 시작. 본문은 그 다음 줄부터.
- 출력은 첫 줄에 관계를 상징하는 은유 제목(7~14자)으로 시작. 대괄호·섹션 태그·식별자는 절대 출력하지 말 것. 총 분량: 2,200~2,800자.
...
- 수치·판정 변경 금지. 흐린 표현 2회 이하.
- 각 ▶ 섹션의 첫 줄은 반드시 은유적 부제목(7~20자)으로 시작. 본문은 그 다음 줄부터.
- 출력은 첫 줄에 관계를 상징하는 은유 제목(7~14자)으로 시작. 대괄호·섹션 태그·식별자는 절대 출력하지 말 것. 총 분량: 2,600~3,400자.
...
- 입력 형식: 글자마다 "뜻 + 음" (예: "허락할 허", "보배 진", "집 우").
...
형식 예시: \`자원오행 판정: 허락할 허→許(言부, 金) · 보배 진→珍(玉부, 金) · 집 우→宇(宀부, 土)\`
